...
# SQL query
query="SELECT t1.Industry_name_NZSIOC, t1.Variable_name AS Variable1, t2.Variable_name AS Variable2, avg(toFloat32OrNull(replaceRegexpAll(t1.Value, ',', ''))) AS AvgValue1, avg(toFloat32OrNull(replaceRegexpAll(t2.Value, ',', ''))) AS AvgValue2, arrayJoin(splitByString(' ', t1.Industry_code_ANZSIC06)) AS SplitIndustryCode, rank() OVER (ORDER BY avg(toFloat32OrNull(replaceRegexpAll(t1.Value, ',', ''))) DESC) AS RankByValue1, rank() OVER (ORDER BY avg(toFloat32OrNull(replaceRegexpAll(t2.Value, ',', ''))) DESC) AS RankByValue2 FROM mydatabase.mytable AS t1 JOIN mydatabase.mytable AS t2 ON t1.Industry_name_NZSIOC = t2.Industry_name_NZSIOC WHERE t1.Year = 2021 AND t2.Year = 2021 AND t1.Variable_name = 'Total income' AND t2.Variable_name = 'Sales, government funding, grants and subsidies' GROUP BY t1.Industry_name_NZSIOC, t1.Variable_name, t2.Variable_name, arrayJoin(splitByString(' ', t1.Industry_code_ANZSIC06)) ORDER BY RankByValue1, RankByValue2 LIMIT 10;"
